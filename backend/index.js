...
const typeDefs = gql`
  ...
  type Mutation {
    ...
    addRoutine(
      name: String
      RoutineDetails: [RoutineDetailsRef]
    ...
    routines: async (_, { filter }) => {
      ...
      const existingRoutinesWithTheirDetails = await Promise.all(
        existingRoutines.map((routine) =>
          Promise.all(
            routine.RoutineDetails.map(({ _id }) =>
              RoutineDetails.findById(_id)
            )
          )
        )
      );
          ...
          for (let i in existingRoutinesWithTheirDetails) {
            existingRoutinesWithTheirDetails[i].map((details) => {
              if (details._id == _id) {
                ...
                existingRoutines[existingRoutinesIndex].RoutineDetails[
                  routineDetailsIndex
                ] = {
                  _id: existingRoutines[existingRoutinesIndex].RoutineDetails[
                    routineDetailsIndex
                  ]._id,
                  ...newDetails,
                };
      ...
      const existingRoutinesWithTheirProducts = await Promise.all(
        existingRoutines.map((routine) =>
          Promise.all(
            routine.RoutineDetails.map(({ products }) =>
              Promise.all(products.map(({ _id }) => Product.findById(_id)))
            )
          )
        )
      );
                ...
                products.map((product) => {
                  ...
                  if (product._id == _id) {
                    ...
                    existingRoutines[existingRoutinesIndex].RoutineDetails[
                      ...
                      _id: existingRoutines[existingRoutinesIndex]
                        .RoutineDetails[routineDetailsIndex].products[
                        productIndex
                      ]._id,
                      ...newProducts,
                    };
  ...
  Mutation: {
    ...
    register: async (_, { username, email, password }) => {
      const createdUser = new User({
        ...
        notifications: {
          routine_id: "1f",
          morning_notification: "9:00",
          night_notification: "18:00",
    ...
    updateNotification: async (
      ...
      try {
        ...
        let userRoutineToBeModified = await User.findOne({ _id: userId }).where(
          { "notifications.routine_id": routine_id }
        );
        if (!userRoutineToBeModified) {
          // let's see if there is any user available and add them that routine :)
          let userExistsToBeUpdated = await User.findById(userId);
          if (userExistsToBeUpdated) {
            userExistsToBeUpdated.notifications.push({
              routine_id,
              morning_notification,
              night_notification,
              custom_notification,
            ...
            if (notifications.routine_id === routine_id) {
              if (morning_notification) {
                await User.findOneAndUpdate(
                  { _id: userId, "notifications.routine_id": routine_id },
                  {
                    "notifications.$.morning_notification": morning_notification
                      ? morning_notification
                      : notifications.morning_notification,
                  }
                );
              }
              if (night_notification) {
                await User.findOneAndUpdate(
                  { _id: userId, "notifications.routine_id": routine_id },
                  {
                    "notifications.$.night_notification": night_notification
                      ? night_notification
                      : notifications.night_notification,
                  }
                );
              }
              if (custom_notification) {
                await User.findOneAndUpdate(
                  { _id: userId, "notifications.routine_id": routine_id },
                  {
                    "notifications.$.custom_notification": custom_notification
                      ? custom_notification
    ...
    removeNotification: async (_, { userId, routine_id }) => {
      ...
      try {
        // let userRoutineToBeModified = await User.findById(userId);
        let userExistsToBeUpdated = await User.findOne({ _id: userId }).where({
          "notifications.routine_id": routine_id,
        });
        if (userExistsToBeUpdated) {
    ...
    addProduct: async (
      _,
      { type, name, title, description, src, refferal }
    ) => {
      if (!name) {
        // !routine_id) {
        return {
    ...
    addDetailsAboutRoutine: async (_, { partOfDay, products }) => {
      ...
      try {
        const createdRoutineWithDetails = new RoutineDetails({
          partOfDay,
          products,
        });
    ...
    addRoutine: async (
      _,
      { name, RoutineDetails, notification_hours, src }
    ) => {
      if (!name) {
        // mmirning) {
        return {
...
mongoose
  .connect(
    `mongodb+srv://${process.env.MONGO_USER}:${process.env.MONGO_PASSWORD}@skc-cluster-mbexs.mongodb.net/skcdb?retryWrites=true&w=majority`
  )
  .then(() => {
